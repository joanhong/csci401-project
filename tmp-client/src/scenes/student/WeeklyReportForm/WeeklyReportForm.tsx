...
submitClicked() {
    ...
    requestEmail.withCredentials = true;
    requestEmail.open('POST', 'http://localhost:8080/users/loggedInUser');
    requestEmail.setRequestHeader('Cache-Control', 'no-cache');
    requestEmail.send();
    var that = this;
    requestEmail.onreadystatechange = function() {
        if (requestEmail.readyState === 4) {
            var response = requestEmail.responseText;
