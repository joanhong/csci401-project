...
public class ProjectAssignment {
	...
	public void importDataFromDatabase() {
		...
		for (Student s : students) {
		    Iterator it = s.getRankings().entrySet().iterator();
		    while (it.hasNext()) {
	            ...
	            rankedProject.setSum_p(rankedProject.getSum_p() + p);
	            rankedProject.setN(rankedProject.getN() + 1);
		    }
		}
        
	}
	public ProjectAssignment(Vector<Project> projects, Vector<Student> students) {
...
//		populateRankingsTable();
	...
	boolean BumpHelper(Student s, int level) {
		...
		Project p = GetProjectWithName(s.getOrderedRankings().get(0));
